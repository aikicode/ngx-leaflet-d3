...
(function(global,factory){typeof exports==="object"&&typeof module!=="undefined"?factory(require("d3"),require("d3-hexbin"),require("leaflet")):typeof define==="function"&&define.amd?define(["d3","d3-hexbin","leaflet"],factory):(global=typeof globalThis!=="undefined"?globalThis:global||self,factory(global.d3,global.d3.hexbin))})(this,function(d3,d3Hexbin){"use strict";function _interopNamespace(e){if(e&&e.__esModule)return e;var n=Object.create(null);if(e){Object.keys(e).forEach(function(k){if(k!=="default"){var d=Object.getOwnPropertyDescriptor(e,k);Object.defineProperty(n,k,d.get?d:{enumerable:true,get:function(){return e[k]}})}})}n["default"]=e;return Object.freeze(n)}var d3__namespace=_interopNamespace(d3);var d3Hexbin__namespace=_interopNamespace(d3Hexbin);
/**
...
this._scale={color:d3__namespace.scaleLinear(),radius:d3__namespace.scaleLinear()};
...
/**
		 ...
		 * @private
		 */
_initContainer:function(){L.SVG.prototype._initContainer.call(this);this._d3Container=d3__namespace.select(this._container).select("g")},
/**
		 * Clean up the svg container
		 * @private
		 */
_destroyContainer:function(){d3__namespace.select(this._container).remove()},
/**
...
// Grid enter-update
gridEnter.merge(join.select("path.hexbin-grid")).on("mouseover",function(event,d,i){that._hoverHandler.mouseover.call(this,that,event,d,i);that._dispatch.call("mouseover",this,event,d,i)}).on("mouseout",function(event,d,i){that._dispatch.call("mouseout",this,event,d,i);that._hoverHandler.mouseout.call(this,that,event,d,i)}).on("click",function(event,d,i){that._dispatch.call("click",this,event,d,i)});
// Exit
...
var tooltip=d3__namespace.select("body").append("div").attr("class","hexbin-tooltip").style("z-index",9999).style("pointer-events","none").style("visibility","hidden").style("position","fixed");tooltip.append("div").attr("class","tooltip-content");
// return the handler instance
return{mouseover:function(hexLayer,event,data){var gCoords=d3__namespace.pointer(event);tooltip.style("visibility","visible").html(options.tooltipContent(data,hexLayer));var div=null;if(null!=tooltip._groups&&tooltip._groups.length>0&&tooltip._groups[0].length>0){div=tooltip._groups[0][0]}var h=div.clientHeight,w=div.clientWidth;tooltip.style("top",""+event.clientY-gCoords[1]-h-16+"px").style("left",""+event.clientX-gCoords[0]-w/2+"px")},mouseout:function(hexLayer,event,data){tooltip.style("visibility","hidden").html()}}},resizeFill:function(){
// return the handler instance
return{mouseover:function(hexLayer,event,data){var o=d3__namespace.select(this.parentNode);o.select("path.hexbin-hexagon").attr("d",function(d){return hexLayer._hexLayout.hexagon(hexLayer.options.radius)})},mouseout:function(hexLayer,event,data){var o=d3__namespace.select(this.parentNode);o.select("path.hexbin-hexagon").attr("d",function(d){return hexLayer._hexLayout.hexagon(hexLayer._scale.radius(hexLayer._fn.radiusValue.call(hexLayer,d)))})}}},resizeScale:function(options){
// merge options with defaults
options=options||{};if(null==options.radiusScale)options.radiusScale=.5;
// return the handler instance
return{mouseover:function(hexLayer,event,data){var o=d3__namespace.select(this.parentNode);o.select("path.hexbin-hexagon").attr("d",function(d){return hexLayer._hexLayout.hexagon(hexLayer._scale.radius.range()[1]*(1+options.radiusScale))})},mouseout:function(hexLayer,event,data){var o=d3__namespace.select(this.parentNode);o.select("path.hexbin-hexagon").attr("d",function(d){return hexLayer._hexLayout.hexagon(hexLayer._scale.radius(hexLayer._fn.radiusValue.call(hexLayer,d)))})}}},compound:function(options){options=options||{};if(null==options.handlers)options.handlers=[L.HexbinHoverHandler.none()];return{mouseover:function(hexLayer,event,data){var that=this;options.handlers.forEach(function(h){h.mouseover.call(that,hexLayer,event,data)})},mouseout:function(hexLayer,event,data){var that=this;options.handlers.forEach(function(h){h.mouseout.call(that,hexLayer,event,data)})}}},none:function(){return{mouseover:function(){},mouseout:function(){}}}};L.hexbinLayer=function(options){return new L.HexbinLayer(options)};
/**
	 * L is defined by the Leaflet library, see git://github.com/Leaflet/Leaflet.git for documentation
	 * We extend L.SVG to take advantage of built-in zoom animations.
...
_destroyContainer:function(){this._d3Container.selectAll("circle").remove();d3__namespace.select(this._container).remove()},
...
if(null==this._map){return this}this._addPing(data,cssClass);this._expirePings();
// Start timer if not active
if(!this._running&&this._data.length>0){this._running=true;this._lastUpdate=Date.now();var that=this;d3__namespace.timer(function(){that._updatePings.call(that,false)})}return this},getActualFps:function(){return this._fps},data:function(){return this._data}});L.pingLayer=function(options){return new L.PingLayer(options)}});
